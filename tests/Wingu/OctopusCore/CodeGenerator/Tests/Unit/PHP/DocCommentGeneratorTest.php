...
class DocCommentGeneratorTest extends TestCase {
    ...
    public function getDataDocCommentsGeneration($calledBy) {
        ...
        $tags = array();
        for($i=0; $i<5; $i++) {
            $mock = $this->getMock('Wingu\OctopusCore\CodeGenerator\PHP\Annotation\Tags\TagInterface', [], [], 'GoodTag'.$calledBy.$i);
    ...
    public function testFromReflection() {
        ...
        $rmAnnotationsCollection = $this->getMock('Wingu\OctopusCore\Reflection\Annotation\AnnotationsCollection', ['getAnnotations'], [], '', false);
        ...
        $reflectionAnnotationTagMock->expects($this->any())->method('getDescription')->will($this->returnValue('my description'));
        $rmAnnotations = [$reflectionAnnotationTagMock];
        $rmAnnotationsCollection->expects($this->any())
            ->method('getAnnotations')
            ->will($this->returnValue($rmAnnotations));

        $reflectionMock = $this->getMock('Wingu\OctopusCore\Reflection\ReflectionDocComment', ['getShortDescription','getLongDescription','getAnnotationsCollection'], [], '', false);
